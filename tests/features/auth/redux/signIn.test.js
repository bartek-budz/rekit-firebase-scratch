...
describe('auth/redux/signIn', () => {
  ...
  it('handles action type AUTH_SIGN_IN_BEGIN correctly', () => {
    const prevState = { loginExistingUserPending: false };
    const state = reducer(
      prevState,
      { type: AUTH_SIGN_IN_BEGIN }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.loginExistingUserPending).toBe(true);
  });

  it('handles action type AUTH_SIGN_IN_SUCCESS correctly', () => {
    const prevState = { loginExistingUserPending: true };
    const state = reducer(
      prevState,
      { type: AUTH_SIGN_IN_SUCCESS, data: {} }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.loginExistingUserPending).toBe(false);
  });
  ...
  it('handles action type AUTH_SIGN_IN_FAILURE correctly', () => {
    ...
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.loginExistingUserPending).toBe(false);
    expect(state.loginExistingUserError).toEqual(expect.anything());
  });

  it('handles action type AUTH_SIGN_IN_DISMISS_ERROR correctly', () => {
    const prevState = { loginExistingUserError: new Error('some error') };
    const state = reducer(
      prevState,
      { type: AUTH_SIGN_IN_DISMISS_ERROR }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.loginExistingUserError).toBe(null);
  });
});
