...
describe('auth/redux/signOut', () => {
  ...
  it('handles action type AUTH_SIGN_OUT_BEGIN correctly', () => {
    const prevState = { logoutUserPending: false };
    const state = reducer(
      prevState,
      { type: AUTH_SIGN_OUT_BEGIN }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.logoutUserPending).toBe(true);
  });

  it('handles action type AUTH_SIGN_OUT_SUCCESS correctly', () => {
    const prevState = { logoutUserPending: true };
    const state = reducer(
      prevState,
      { type: AUTH_SIGN_OUT_SUCCESS, data: {} }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.logoutUserPending).toBe(false);
  });
  ...
  it('handles action type AUTH_SIGN_OUT_FAILURE correctly', () => {
    ...
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.logoutUserPending).toBe(false);
    expect(state.logoutUserError).toEqual(expect.anything());
  });

  it('handles action type AUTH_SIGN_OUT_DISMISS_ERROR correctly', () => {
    const prevState = { logoutUserError: new Error('some error') };
    const state = reducer(
      prevState,
      { type: AUTH_SIGN_OUT_DISMISS_ERROR }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.logoutUserError).toBe(null);
  });
});
